...
public partial class Main : Node2D
{
    ...
    private void UpdateLabels()
    {
        ...
        _runtimeStatsLabel.Text =
            ...
            $"Kin scent {FormatPercent(Share(snapshot.CreatureSimilarityScentDetectedCreatureCount, snapshot.CreatureCount))}  lineage {FormatPercent(Share(snapshot.CreatureLineageScentDetectedCreatureCount, snapshot.CreatureCount))}  identity {FormatPercent(Share(snapshot.CreatureIdentityScentDetectedCreatureCount, snapshot.CreatureCount))}  egg {FormatPercent(Share(snapshot.EggLineageScentDetectedCreatureCount, snapshot.CreatureCount))}/{FormatPercent(Share(snapshot.EggIdentityScentDetectedCreatureCount, snapshot.CreatureCount))}  density {snapshot.AverageCreatureSimilarityScentDensity:0.00}/{snapshot.AverageCreatureLineageScentDensity:0.00}/{snapshot.AverageCreatureIdentityScentDensity:0.00}/{snapshot.AverageEggLineageScentDensity:0.00}/{snapshot.AverageEggIdentityScentDensity:0.00}\n" +
            ...
            $"Similar/lineage/identity touch {FormatPercent(Share(snapshot.SimilarCreatureContactCreatureCount, snapshot.CreatureCount))}/{FormatPercent(Share(snapshot.LineageCreatureContactCreatureCount, snapshot.CreatureCount))}/{FormatPercent(Share(snapshot.IdentityCreatureContactCreatureCount, snapshot.CreatureCount))}  intent {FormatPercent(Share(snapshot.AttackIntentWhileTouchingSimilarCreatureCount, snapshot.CreatureCount))}/{FormatPercent(Share(snapshot.AttackIntentWhileTouchingLineageCreatureCount, snapshot.CreatureCount))}  avg {snapshot.AverageCreatureContactSimilarity:0.00}/{snapshot.AverageCreatureContactLineageSimilarity:0.00}/{snapshot.AverageCreatureContactIdentitySimilarity:0.00}\n" +
