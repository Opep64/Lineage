...
public sealed class NeuralBrainGenome
{
    ...
    private const int LegacyInputCountWithoutMeatPayoff = NeuralBrainSchema.RecentMeatRawYieldInput;
    ...
    private static (float[] Weights, int HiddenNodeCount) NormalizeWeights(float[] weights)
    {
        ...
        if (TryInferLegacyWeightLayout(
            weights.Length,
            LegacyInputCountWithoutMeatPayoff,
        ...
        int oldReproductionReadinessInput)
    {
        ...
        if (legacyInputCount == LegacyInputCountWithoutMeatPayoff)
        {
            return input;
        }
