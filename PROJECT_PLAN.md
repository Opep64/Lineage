...
- Rotten meat is now locally perceptible but imperfect: visible meat exposes a freshness cue, stale carcasses emit a separate rot scent, neural brains receive freshness/rot-scent inputs, and CSV/HTML/Godot diagnostics report fresh meat seen, stale meat seen, stale seen-but-not-eaten, rot scent, and rot scent density.
- CLI and Godot HTML reports now include species-cluster freshness/rot wiring diagnostics, so carrion-pressure runs can show whether a surviving cluster is actually wiring the freshness cue or rot-scent inputs before a behavioral assay label changes.
- Rot/carrion probe output now includes perception-specific tail metrics and rotten-meat behavior labels. A 2026-05-23 probe across Scavenger, Omnivore, and Predation Pressure showed stronger scent/persistence/damage variants increasing rot-scent exposure and stale intake, but behavior assays still reported `little freshness differentiation` and carrion adaptation only nudged into roughly the `0.003-0.007` tail-average range in the strongest 20k combined runs. No scenario JSON tuning was applied from that pass.
- Fresh-vs-stale behavior assays now include paired right/ahead stale-meat and rot-scent probes, plus numeric fresh-meat preference and rot-scent avoidance scores in full reports and lightweight probes. A 2026-05-23 5k two-seed diagnostic probe found fresh-preference score still `0` across Scavenger, Omnivore, and Predation Pressure; rot-scent avoidance was only about `0.05-0.06` for Scavenger/Predation starters and `0` for Omnivore. This confirms the current starter/evolved populations are not yet using the freshness cue in a meaningful way.
- Added a probe-only `FreshnessAwareScavenger` starter brain that follows normal scavenger meat cues but suppresses movement/turning into rot scent. A 2026-05-23 20k three-seed probe compared current starters, stale-pressure variants, and freshness-aware variants. In the controlled Scavenger Pressure comparison, freshness-aware scavengers improved final population from `101.0` to `130.7` in base conditions and from `81.7` to `102.3` in stale-pressure conditions, while reducing tail stale intake to `0%` and lowering rotten damage from `0.0045-0.0103` health/s to about `0.0018` health/s. Omnivore and Predation variants also improved population, but those comparisons changed starter family and should not be interpreted as pure freshness effects.
- Added `scenarios/carrion-pressure.json` as a dedicated decay-pressure preset. A 2026-05-23 20k three-seed probe selected it from Scavenger Pressure variants: compared with Scavenger base it lowered final population from `101.0` to `53.7`, raised tail meat intake from `9.8%` to `13.2%`, raised tail stale-carcass intake from `0%` to `12.1%`, raised rot-scent exposure from `31.2%` to `48.2%`, and raised rotten damage from `0.0038` to `0.0119` health/s while remaining viable. The same settings with `FreshnessAwareScavenger` improved final population to `81.7` and cut tail stale intake to `0%`, suggesting decay can matter if behavior can discover or retain the cue.
- A 2026-05-24 20k three-seed report probe used the new species-cluster freshness wiring diagnostics on Scavenger Pressure and Carrion Pressure. Carrion Pressure remained harsher than Scavenger (`37.0` vs `64.7` average final creatures), raised rot-scent exposure (`48.1%` vs `40.7%`) and rotten damage (`0.01076` vs `0.00612` health/s), but both scenarios still collapsed to one living species cluster per run and showed only tiny freshness/rot wiring magnitudes (`0.001-0.008`). This suggests the pressure is real but not yet discoverable enough under current generation counts; the next carrion pass should create more viable evolutionary opportunity rather than only making carrion more punishing.
- A follow-up 2026-05-24 carrion tuning probe promoted a more survivable Carrion Pressure window: plant density `92/M`, regrowth `0.25-1.25`, plant dormancy `50-150s`, reproduction threshold `110`, offspring investment `40`, egg production `1.25/s`, maturity `140s`, cooldown `36s`, rotten damage `0.0055`, mutation strength `0.08`, trait mutation `0.24`, and brain mutation `0.11`. Across seeds 42-44 this improved average final population from `37.0` to `53.0` and average max generation from `2.0` to `2.7` while retaining measurable stale intake (`9.7%`), rot-scent exposure (`43.5%`), and rotten damage (`0.00963` health/s). This is still not enough to show strong freshness wiring, but it gives carrion selection a larger viable window.
- A longer 2026-05-24 Carrion Pressure check at 60k ticks showed the promoted window is not yet long-run stable: seeds 42, 43, and 44 ended at `12`, `0`, and `9` creatures, with one extinction before 60k. Surviving runs had more visible wiring than the 20k check (`0.01-0.03` direct freshness/rot magnitude in the surviving clusters), but behavior assays still reported little freshness differentiation and the tail populations were only about `9-12` creatures. The next Carrion Pressure pass should focus on preventing late starvation collapse, likely by improving food/offspring survival balance or adding a softer recovery mechanism, before expecting robust freshness adaptation.
...
- Add scenario pressure presets. Gentle, balanced, harsh, scavenger-pressure, carrion-pressure, omnivore-pressure, and predation-pressure presets done.
...
- Add first carrion pressure preset. `scenarios/carrion-pressure.json` starts from scavenger pressure but lowers plant reliability, slows plant respawn, makes carcasses richer and more persistent, raises rotten-meat damage, and strengthens meat/rot scent. It is intended as a dedicated decay-pressure experiment where stale-meat choices are measurable without making every preset harder. Done.
...
- Retune Predation Pressure as the first "evolution opportunity" preset. A 2026-05-24 60k-tick probe promoted the former `fast_mut` variant into `scenarios/predation-pressure.json`: maturity age `115s`, reproduction cooldown `30s`, egg incubation `26s`, reproduction threshold `104`, offspring investment `36`, mutation strength `0.11`, trait mutation rate `0.28`, and brain mutation rate `0.16`. Seeds 42-44 stayed viable, reached generation 5-8, split into multiple small species clusters, and produced repeatable notable attack-response shifts. Scavenger variants reached more generations but did not show notable behavior shifts, so scavenger pressure should next get a mechanics pass rather than only faster generations. Done.
...
- Make sufficiently decayed meat actively risky for creatures that are not adapted to it. Eating old carcasses causes digestion-time health loss, while carrion-specialized lineages can tolerate most of that damage. Done.
- Add imperfect stale-meat perception. Visible meat now carries a freshness cue and stale carcasses emit a distinct rot scent, letting carrion eaters evolve around rotten food without giving creatures a global or exact meat-age value. Done.
...
- Probe stronger biome movement-cost pressure before changing presets. A 2026-05-22 broad 5k diagnostic compared base against `move-medium`, `move-hard`, and `move-basal`; a 10k three-seed focused pass compared base against `move-hard`; then a 10k three-seed pass compared base against `rough-no-rich` where poor biomes were expensive but rich biomes stayed near neutral. No scenario JSON changes were applied: all candidates were viable, but stronger energy-only biome costs did not consistently add useful pressure. Balanced got easier in the 10k passes (`654` base average final population versus `704` for `move-hard` and `695` for `rough-no-rich`), while harsh and scavenger became only modestly harder. This suggests biome calorie cost alone is too blunt, and the next terrain-pressure step should test speed/drag or stronger local terrain effects rather than just retuning the existing energy multipliers. Probe files: `out/biome_pressure_variant_probe_20260522.csv`, `out/biome_pressure_move_hard_probe_20260522.csv`, and `out/biome_pressure_rough_no_rich_probe_20260522.csv` with matching `.html` reports. Done.
...
- Treat obstacles, costly terrain, and hazards as related but separate pressure layers. Impassable obstacles such as mountains, cliffs, deep water, walls, or dense rock should block travel and possibly vision/scent. Costly terrain such as hills, wetlands, shallow water, snow, sand, or mud should remain passable but slower or more expensive. Hazards such as lava, fire, toxins, freezing zones, radiation, thorns, tar pits, or disease pools should be enterable but damaging, energy-draining, immobilizing, or lethal over time.
- Creatures should not receive direct omniscient labels such as `LavaAhead` or `WallAhead`. Prefer local imperfect cues: blocked/current movement failed, forward/left/right obstruction feelers, current and forward terrain drag, heat/cold/toxic scent gradients, recent pain, current damage rate, dead-creature density, wetness/traction, and possibly sound or smell spread around obstacles.
- Initial obstacle interaction can be simple collision plus slide/stall movement. A creature attempting to move into an obstacle loses forward progress, gets a blocked/recent-failed-movement cue, and can evolve to turn away or follow edges. Later memory and hidden nodes may be needed for sophisticated detours, wall-following, mountain-pass navigation, or returning to known safe corridors.
- Consider lightweight creature-creature body collision later, using a spatial-grid broad phase rather than all-pairs checks. The first version could resolve overlapping circular bodies with small deterministic push-apart corrections and expose crowding or stalled-contact cues. A later collision-damage layer could make impacts matter: damage or displacement should scale with relative speed, body-size/mass difference, angle, and defensive traits, so a large fast creature colliding with a much smaller one can injure or kill it while taking little damage itself. Keep this separate from intentional biting/attack behavior so accidental trampling, crowding, blocking, and stampede-like pressure can evolve as their own ecological forces.
- Initial hazard interaction can be local exposure plus damage. For example, lava would emit heat before contact, high heat and damage while inside it, and increasing risk with longer exposure. Evolution could discover heat avoidance, cautious edge following, fast risky crossing, or tolerance traits if the scenario rewards them.
- Useful future obstacle/hazard scenarios include risky shortcuts across damaging terrain, mountain ranges with narrow passes, seasonal bridges or frozen/dry corridors, rich food near danger, dead-zone warning areas where carcasses indirectly reveal danger, shelters that protect from weather/predators, and chokepoints that test navigation and crowding behavior.
...
- Future world generation should support a more believable generated-world option in addition to the current noise and band maps, especially once larger worlds are practical. Start with a lightweight continental-style generator rather than a full erosion/weather simulator: broad elevation, coarse water/lake/sea masks, moisture, temperature, fertility, terrain cost, and optional hazard/obstacle layers. Biome labels should be derived from those layers rather than being the only underlying truth.
- Lakes and seas should be supported early in that generator as impassable or near-impassable areas, because they create meaningful barriers, coastlines, corridors, and isolation without requiring rivers, erosion, or active weather simulation. Later, shallow water, wetlands, seasonal ice, or drying corridors could become passable-but-costly terrain instead of hard obstacles.
- A first believable generator could combine broad elevation noise, simple sea/lake thresholds, latitude-plus-elevation temperature, moisture from distance-to-water plus noise, and biome classification from temperature/moisture/elevation. This should create coasts, dry interiors, fertile valleys, highlands, wetlands, plains, forests, tundra, deserts, and mountain barriers well enough for ecological experiments.
- Keep generated-world layers deterministic and editable. Manual world painting should work as overrides on top of generated layers, allowing the user to add mountain walls, lakes, swamps, fertile valleys, hazard zones, refuges, or migration corridors after generation.
- Do not make `BiomeCellSize` carry every future terrain detail. Biome cells can remain coarse for climate/fertility, while future passability, lakes/seas, hazards, and authored terrain masks may need separate layers or masks so coastlines and obstacles can be finer without making every biome lookup/draw cell tiny. Current biome lookup is cheap per query, but very small biome cells increase map memory, generation/setup work, resource spawn prefix-table size, and Godot biome-overlay drawing cost.
...
- Add internal health and recent-damage senses before hazards or predator-heavy ecosystems become central. Useful inputs could include current health ratio, recent damage rate, recent attack damage, recent hazard/rot damage, or a generic pain/stress signal. These should feed the brain as cues, not force behavior, so evolution can discover avoidance, fleeing, defensive posture, disengaging from fights, avoiding hazardous terrain, or tolerating damage when the reward is high.
...
- Start future plant diversity with ecological archetypes rather than real species. A useful first set would be grass/ground cover as common low-calorie fast-regrowth food, leafy herbs as soft medium-value forage, berry bushes or fruiting shrubs as high-value intermittent food with stronger smell/color cues, tough woody shrubs as slower/harder food that may favor larger bodies or stronger bites, and bitter/toxic plants as visible food that is not always worth eating. Later additions could include fruit trees as persistent landmark/obstacle resources, tubers/root plants that require smell or digging/time, succulents/water-rich plants for dry or hot pressure, and seasonal bloom plants for feast/famine experiments.
- Keep future plant/resource categories functional rather than species-specific. There is no simulation need to distinguish strawberry from blueberry or onion from carrot at first; use classes such as grass/ground cover, leafy herb, berry/fruit, seed/nut, tuber/root, mushroom/fungus, succulent, woody shrub, bitter/toxic plant, tree products, insects, carrion, and hives. Expose coarse estimated amount/value such as small/medium/large or normalized density so larger creatures can learn to ignore low-density snacks unless hungry or no better food is detectable.
- Do not give creatures perfect truth about plant/resource value. Prefer imperfect cues such as food class/archetype, approximate size or calorie amount, scent/color strength, toughness/access cost, bitterness/toxicity/risk cues, freshness/decay, and estimated payoff filtered through the creature's genes. Specialized genes can make repeated use of a class more profitable, such as berry/fruit efficiency, tuber digestion, mushroom tolerance, insect digestion, honey/sugar efficiency, toxic tolerance, or woody/tough-plant access.
- Trees should eventually be modeled as persistent large plant-obstacles or habitat features, not just calorie circles. They may block movement and vision, drop fruit/seeds/leaves, create shade or local habitat, and germinate into new trees only slowly. Do not enable unlimited tree recruitment without removal pressures such as age/senescence, fire, drought, disease, insect infestation, storms, specialized herbivory, or scenario/editor pruning, otherwise long runs could gradually clog the map.
- Static insects can act as meat resources without the cost of simulating many moving animals. Candidate forms include ant/termite mounds, grub logs, insect swarm/cloud patches, carrion insects, and swamp/forest insect abundance. Most insects should count as meat. Bee hives are a special mixed resource: bee/larva meat, honey as plant-like/simple-sugar calories, and sting damage or access risk while feeding unless the creature specializes.
- Keep food perception and brain inputs compressed for performance. Avoid one brain input per visible resource instance or per real plant species. Start with either a small number of archetype channels, such as proximity/forward/right/density per broad food class, or top-K visible food candidate slots carrying compact features like approximate class, distance, direction, size/value, toughness, toxicity/risk, and estimated digestible payoff. This lets creatures choose between several visible foods without making brain evaluation or sensing scale with every plant on screen.
- Treat resource diversity as a large-world performance problem as much as an ecology problem. Rich plant classes, insects, hives, trees, seeds, and future propagation only help if the simulator can still run landmasses large enough for isolation and diverse regions. Favor sparse/static resource indexes, active-region or lazy cell indexing, chunk summaries, coarse biome/terrain layers, and versioned or incremental updates so cost tracks active ecology near creatures rather than total map area.
...
- Add an analysis/reporting layer that classifies evolved creatures without pretending the categories are absolute biological species. First pass done: `SpeciesClusterAnalyzer` groups living creatures by genome trait similarity plus neural-brain weight similarity, assigns deterministic readable cluster names, and writes the results to CLI/Godot CSV and HTML reports. Cluster history is also reconstructed from lineage records and stats snapshots, then written to species trend CSVs and HTML report tables without affecting simulation behavior. Reports now include lifecycle labels, diversity-over-time metrics, a compact diversity chart for active clusters, dominant-cluster share, sampled turnover, cluster-level behavior fingerprints, cluster-level freshness/rot input wiring diagnostics, early-versus-final behavior change comparisons with a compact notable-shifts summary, and a "why these clusters matter" interpretation table that combines role, founder ancestry, survival trend, and supporting evidence.
...
- A 2026-05-24 cluster-quality smoke pass ran 12k ticks at seed 42 for Balanced, Terrain Pressure, Migration Pressure, Scavenger Pressure, and Predation Pressure. All five remained a single active species cluster through the sampled history, with final populations matching the synthetic final history tick. This suggests current thresholds are not over-splitting short low-generation runs; reports now flag these as low-differentiation `persistent dominant` populations rather than implying a meaningful species split.
- A 2026-05-24 divergent validation pass reinterpreted saved snapshots from a mixed starter roster and a longer Balanced run. Tightening the default brain-distance threshold to `0.028` split the 20k mixed roster into three active clusters (200 plant-forager family, 21 scavenger-ish survivors, 5 predator survivors) while the 60k Balanced run stayed a single active cluster at generation 6. Current species summaries now use the same lineage cluster identities as the history table, so cluster names stay aligned between the current CSV/report and the trend CSV/report.
...
- Add a near-term generic intent feedback mechanism that feeds back into the brain without taking control. Track recent intended effort versus achieved progress as continuous state on the creature, then expose inputs such as `IntentProgress` and `IntentFrustration`. Keep this as generic as practical rather than immediately adding separate food/move/reproduce failure inputs. The 2026-05-23 obstacle runs showed creatures getting stuck near reachable-looking obstacle-edge food because strong food cues kept winning even when no calories were gained and movement progress was tiny. A generic frustration signal should let evolution discover behaviors such as abandoning unrewarding local targets, turning away from obstacle-food traps, or resuming search, while still leaving the final action choice to the brain.
...
- Future egg recognition should explore distinctions between a creature's own eggs, close-kin or same-species eggs, and other-species eggs. Avoid direct labels like `OwnEgg` or `SameSpeciesEgg`; prefer imperfect cues such as egg scent/marking, nest or location context, recent laying memory, genetic/lineage odor similarity, egg health/freshness, and repeated local familiarity. This would let evolution discover strategies such as guarding offspring, tolerating kin eggs, raiding competitors' eggs, or reclaiming doomed eggs as emergency calories.
...
- Keep the future grab/attach mechanic generic enough to support carrying non-creature targets. A creature might grab a portable plant part, small plant, egg, carcass, or other resource, then move away while eating, caching, stealing, or protecting it. Rooted or large plants should resist carrying, while fruit, leaves, tubers, small plants, eggs, and meat could have size/anchoring/weight rules. Held items should add movement or energy cost, expose held-target senses, allow drop/release behavior, and interact with eating so animals can flee competition or danger while consuming food elsewhere.
...
Next practical step: use Carrion Pressure to make freshness-aware behavior discoverable by evolution rather than only available as a handcrafted starter. The controlled Scavenger and Carrion Pressure probes show the ecology can reward avoiding stale/rotten cues, but the normal starter does not discover that behavior in short runs. Good next candidates are adding a report/selection diagnostic for freshness-input weight magnitude by lineage, increasing mutation/structural routes into freshness and rot-scent inputs, or running mixed-species competition where a freshness-aware mutant can appear alongside normal scavengers without being prewired into every founder.

Memory note to keep: creatures have one persistent world-space memory vector with brain inputs/outputs, scenario-backed decay/write-rate/upkeep settings, CSV telemetry, selected-creature viewer display, and memory-user versus non-memory-user effectiveness diagnostics in CSV/HTML reports. A 2026-05-23 three-seed 60k tick probe showed memory emerging by mutation in both terrain-pressure and migration-pressure runs despite zero memory wiring in the handcrafted starter brains. Terrain-pressure ended with `13.5%`, `27.9%`, and `27.0%` active-memory creatures across seeds 42-44; migration-pressure ended with `26.3%`, `31.3%`, and `30.8%`. Average memory strength stayed low, roughly `0.024-0.034` over the tail. A follow-up effectiveness probe showed migration memory users outperforming non-memory users on raw kcal/unit (`1.572` vs `0.210`), meal gap (`21.6s` vs `33.2s`), food success (`22.5%` vs `12.2%`), generation (`2.46` vs `1.57`), average max-X progress (`63.5%` vs `50.6%`), and right-region share (`53.8%` vs `33.3%`). A one-seed 60k tuning sweep compared base memory settings against higher cost, faster decay, slower write, and a combined firmer variant; the base settings still had the strongest migration route signal, so no default scenario tuning or starter prewiring was applied.
...
- `SimulationStatsSnapshot`: per-sample population/resource/lineage/egg metrics plus biome occupancy, per-biome plant/meat calories, per-biome eating, per-biome deaths, eastward crossing progress, environmental cost, foraging, gut/digestion, and predator/prey diagnostics.
...
- visible meat freshness
- rotten meat scent density
- rotten meat scent direction forward
- rotten meat scent direction right
- dietary meat bias
- egg reserve ratio
...
Build, 111 core tests, CLI scavenger-pressure rotten-sensing smoke/report/snapshot, and Godot headless `--quit` passed on 2026-05-23 after adding visible meat freshness, rot scent inputs, behavior-assay classification, neural migration for 38-input brains, and stale-meat diagnostics in CSV/HTML/Godot.
Build, 111 core tests, `git diff --check`, a 45-run 10k rot/carrion probe, and an 18-run 20k combined stale-pressure probe passed on 2026-05-23 after adding rot/carrion columns to the lightweight probe CSV/report. Probe files: `out/rot_carrion_probe_20260523/rot_carrion_probe_20260523.csv`, `.html`, `rot_carrion_combined_20k_20260523.csv`, and `.html`.
Build, 113 core tests, a 12-run fresh/stale choice diagnostic probe, and probe CSV header/value sanity checks passed on 2026-05-23 after adding paired fresh-vs-stale behavior scores to reports/probes. Probe files: `out/fresh_stale_choice_probe_20260523/fresh_stale_choice_probe_20260523.csv` and `.html`.
Build, 114 core tests, and a 36-run 20k freshness-aware starter comparison probe passed on 2026-05-23 after adding the `FreshnessAwareScavenger` starter brain. Probe files: `out/freshness_aware_probe_20260523/freshness_aware_probe_20260523.csv` and `.html`.
Build, 114 core tests, CLI Carrion Pressure smoke/report, Godot headless `--quit`, `git diff --check`, an 8-run 10k carrion-pressure tuning probe, and a 12-run 20k selection probe passed on 2026-05-23 after adding `scenarios/carrion-pressure.json`. Probe files: `out/carrion_pressure_tuning_20260523/carrion_pressure_10k_probe.csv`, `.html`, `carrion_pressure_20k_probe.csv`, and `.html`.
...
- carrion adaptation trading fresh meat efficiency for stale meat recovery
